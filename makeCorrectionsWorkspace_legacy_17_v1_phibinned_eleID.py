...
for task in histsToWrap:
    if not ("ID90" in task[0] or "ID80" in task[0]):
        wsptools.SafeWrapHist(w, ['e_pt', 'expr::e_abs_eta("TMath::Abs(@0)",e_eta[0])'],
                          GetFromTFile(task[0]), name=task[1])
    else:
        wsptools.SafeWrapHist(w, ['e_pt', 'expr::e_abs_eta("TMath::Abs(@0)",e_eta[0])', 'e_phi'],
                  GetFromTFile(task[0]), name=task[1])
...
w.Print()
w.writeToFile('htt_scalefactors_v17_eleID_phi.root')
w.Delete()
